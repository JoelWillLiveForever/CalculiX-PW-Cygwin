...
!
c            weldMatNumber = -1    
c            weldpass=(istep+2)/3 
c            if ((matname(imat)(1:1)).eq.'W') then
c                read(matname(imat)(2:4),'(i3)') weldMatNumber 
c            endif
c!            if (istep.eq.1) then
c!                print *,"istep:", istep, "imat: ", imat
c!            endif
c!            if (imat.eq.3) then
c!                print *,"istep:", istep, "imat: ", imat, "weldMatNum",
c!     &              weldMatNumber
c!            endif
c
c!            print *,"weldMatNumber: ", weldMatNumber
c            if(weldMatNumber.eq.1) then
c               if(weldpass.lt.1) then
c                  fa=0.000000000001
c                  print *,"istep:", istep, "imat: ", imat
c!                        write(7,*) "time ",time
c!                        write(7,*) "istep ",istep
c!                        write(7,*) "matname ",matname(imat)
c                  do ii=1,6
c                     coconloc(ii)=coconloc(ii)*fa
c                  enddo
c               endif
c            endif
c!!                
c            if(weldMatNumber.eq.2) then
c               if(weldpass.lt.2) then
c!                   if (istep.eq.1) then
c!                       print *,"istep:", istep, "imat: ", imat
c!                   endif 
c                  fa=0.000000000001
