...
!
     ...
     &  pslavsurf(3,*),pmastsurf(6,*)
      ...
      real*8 fa
      integer ii, weldpass, iPass, totalPasses
      character(3) weldPassStr
      character(4) weldMatName
!
      include "gauss.f"
!
      iflag=3
...
! 
!            Deactivate the welding passes that have not been deposited yet.
!
!     
!      weldpass=(istep+2)/3 
!      totalPasses = 2
!      fa = 0.0001
!      do iPass=1,totalPasses
!          write(weldPassStr,'(I0)' )   iPass
!          weldMatName = "W"//weldPassStr
!          if(matname(imat).eq.weldMatName) then
!              if(weldpass.lt.iPass) then
!!                  write(*,*) "time ",time
!!                  write(*,*) "istep ",istep
!!                  write(*,*) "matname ",matname(imat)
!                  do ii=1,ntmat_
!                      elcon(1,ii,imat)=elcon(1,ii,imat)*fa
!                  enddo
!              endif
!          endif
!      enddo
!      if(matname(imat).eq.'W1') then
!          if(weldpass.lt.1) then
!!             write(7,*) "time ",time
!!             write(7,*) "istep ",istep
!!             write(7,*) "matname ",matname(imat)
!              do ii=1,ntmat_
!                  elcon(1,ii,imat)=elcon(1,ii,imat)*fa
!              enddo
!          endif
!      endif
!                
!      if(matname(imat).eq.'W2') then
!          if(weldpass.lt.2) then
!!             write(7,*) "time ",time
!!             write(7,*) "istep ",istep
!!             write(7,*) "matname ",matname(imat)
!              do ii=1,ntmat_
...
!              enddo
!          endif
!          if(weldpass.eq.2)then
!              elcon(1,ii,imat) = elcon(1,ii,imat)
!          endif
!      endif
